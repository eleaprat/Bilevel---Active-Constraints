...
def VarLower(n_bus, Demand,n_DT,data,mode):
   ...
   # 1 - Prepare identification of the sets of active constraints
    ...
    if os.path.exists(name_AS):
        ...
        sys.exit('The list of active sets in csv is not available. Please add the file {} to the current directory or create it with the script VarLower_DB_DiscoverMass.py'.format(name_AS))

    # list out keys and values separately 
    AS_key = list(AS.keys()) 
    AS_val = list(AS.values()) 
    ...
    # 3 - Output of DTs for this load
    dt_output=[]
    for i in range(n_DT):
        name_DT = 'VarLower_DT_{}bus_sub{}.pkl'.format(n_bus,i+1)
        if os.path.exists(name_DT):
            ...
            if mode == 'basic':
                ...
                if os.path.exists('Classes_{}'.format(name_DT)):
                    ...
                    sys.exit('Please add the file {} to the directory and run again. If it does not exist, run VarLower_DT_Generation.py first. '.format('Classes_{}.pkl'.format(name_DT)))
            else:
                sys.exit('{} is not an accepted mode. Please select a proper mode: "basic" or "enhanced" and run again.'.format(mode))
        else:
            sys.exit('The decision tree {} does not exist. Please make sure that the proper number of decision trees have been entered and that the files have been added to the directory. If they do not exist, run VarLower_DT_Generation.py to create them.'.format(name_DT))
        ...
        All_Sets=[]
        for i in range(len(dt_output)):
            All_Sets.append(AS_key[AS_val.index(dt_output[i])])
    
    # 4 - Run one LP for each active set identified
    ...
    for Act_cstr in All_Sets:
        
        start_time_loop = time.time()
        k+=1
                
        # Model alias
        model = gb.Model('VarLower_Bilevel_{}'.format(k))
    ...
    #-------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # model.write('VarLower_Bilevel_{}_Model.lp'.format(k)) # Equations
        # model.write('VarLower_Bilevel_{}_Results.sol'.format(k)) # Results
