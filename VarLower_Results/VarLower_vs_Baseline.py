...
from Bilevel_VarLower import VarLower
...
n_DT = 2 # Number of decision trees from the previous step
...
File_name = 'VarLower_Results_{}bus_{}.csv'.format(n_bus,mode)
Headline = ['Load','Duration BL (s)','Duration VL (s)',
                        'Obj fct BL','Obj fct VL','Optimality gap (%)', 'Recovered from intractable',
                        'Cost cS BL','Cost cS VL',
                        'alpha1 BL','alpha1 VL',
                        'Pg1 BL','Pg1 VL',
                        'Nb bin BL','Nb LPs VL',
                        'Nb cstr BL','Nb eq BL',
                        'Nb ineq BL','Nb cstr VL']
...
# 2 - Run Optimization
                ...
                Res_meth,dur_meth = VarLower(n_bus,Demand,n_DT,data,mode)
...
# And with the method
intract_meth = False
if 'NaN_9' in list(df['Obj fct VL'].values):
    n_intract_meth=df['Obj fct VL'].value_counts()['NaN_9']
    intract_meth = True
...
feas_list=[str(x) for x in df['Obj fct VL'].tolist()]
n_infeas=sum('NaN' in i for i in feas_list)


# Printed results
print('-------------------------------------------------------------------------------------------------------', end='\n')
print('---------- Results for a {} bus system with the method VarLower, with {} points tested ----------'.format(n_bus,test_pts), end='\n')
print('-------------------------------------------------------------------------------------------------------', end='\n \n')
print('Baseline:', end='\n * ')
print('Duration:', end='\n    * ')
...
print('VarLower:', end='\n * ')
print('Duration:', end='\n    * ')
print('Average: {}s'.format(round(df['Duration VL (s)'].mean(),3)), end='\n    * ')
print('Min: {}s'.format(round(df['Duration VL (s)'].min(),3)), end='\n    * ')
print('Max: {}s'.format(round(df['Duration VL (s)'].max(),3)), end='\n    * ')
print('Compared to baseline: {}%'.format(round(100*df['Duration VL (s)'].mean()/df['Duration BL (s)'].mean(),1)), end='\n    * ')
if intract_meth == True:
    print('Intractable cases: {} ({}%)'.format(n_intract_meth,round(100*n_intract_meth/test_pts,1)), end='\n * ')
else:
    print('No intractable cases', end='\n * ')
print('Number of LPs solved: {}'.format(int(df['Nb LPs VL'].mean())), end='\n * ')
print('Number of constraints (equalities): {}'.format(int(df['Nb cstr VL'].mean())), end='\n * ')
...
dur_M = pd.DataFrame({'Duration (s)':df['Duration VL (s)'], 'Test case':'{} bus'.format(n_bus), 'Solving method':'VarLower'})
...
gold = '#C48820'
blue = '#166FA0'
pal = {'Baseline': blue, 'VarLower': gold}
hue_order= ['Baseline', 'VarLower']
