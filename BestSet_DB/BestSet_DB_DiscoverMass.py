...
if os.path.exists(name_DB): 
    ...
    Active_Sets = {AS_tuple[k]:set_nb[k] for k in range(len(AS_tuple)) if AS_tuple[k] is not None}
...
name_DM = 'BestSet_DiscoverMass_{}bus.pkl'.format(n_bus) 
...
# Get DCOPF results for each new sample
        ...
        best={} # Dictionnary to store results for the best value of cg1
        ...
        for cS in CS:
            ...
            if feas == 2:
                ...
                # Check if the value of the profit of the strategic producer is better for this point, if so replace
                if not best: # If best is empty, replace with the results of this run
                    best = primal
                else:
                    if (primal['P_g1']*(primal[alpha_str]-cS_min))>(best['P_g1']*(best[alpha_str]-cS_min)): 
                        best=primal
        ...
        if best: # If best is not empty
            # Build DB only for the best point
            active_cstr_temp = []
            # Creation of the set of active constraints
            for c in cstr:
                if best[c] > epsilon or best[c] < - epsilon: # If the dual variable is not zero
