...
from Bilevel_AllSets import AllSets
...
mode = 'enhanced' # 'basic' to use the output of the DT
...
File_name = 'AllSets_Results_{}bus_{}.csv'.format(n_bus,mode)
Headline = ['Load','Duration BL (s)','Duration AS (s)',
                        'Obj fct BL','Obj fct AS','Optimality gap (%)', 'Recovered from intractable',
                        'Cost cS BL','Cost cS AS',
                        'alpha1 BL','alpha1 AS',
                        'Pg1 BL','Pg1 AS',
                        'Nb bin BL','Nb LPs AS',
                        'Nb cstr BL','Nb eq BL',
                        'Nb ineq BL','Nb cstr AS']
...
while len(TEST) < test_pts:
    ...
    if sum(Demand) <= load_max and sum(Demand) >= load_min: # Check that the total load is between the limits
        if Demand not in TEST: # Check that this point does not already exist
            ...
            (primal, duals_ineq)= DCOPF(data) # Run DCOPF with any cg1 cost to ensure feasibility
            feas = primal['feas']
            if feas == 2:  # Only run the methods if the point is feasible
                TEST.append(Demand)
# 2 - Run Optimization
                Res_bl,dur_bl = Baseline(data,n_bus)
                Res_meth,dur_meth = AllSets(n_bus,Demand,data,mode)
...
# And with the method
intract_meth = False
if 'NaN_9' in list(df['Obj fct AS'].values):
    n_intract_meth=df['Obj fct AS'].value_counts()['NaN_9']
    intract_meth = True
...
feas_list=[str(x) for x in df['Obj fct AS'].tolist()]
n_infeas=sum('NaN' in i for i in feas_list)


# Printed results
print('-------------------------------------------------------------------------------------------------------', end='\n')
print('---------- Results for a {} bus system with the method AllSets, with {} points tested ----------'.format(n_bus,test_pts), end='\n')
print('-------------------------------------------------------------------------------------------------------', end='\n \n')
print('Baseline:', end='\n * ')
print('Duration:', end='\n    * ')
...
print('AllSets:', end='\n * ')
print('Duration:', end='\n    * ')
print('Average: {}s'.format(round(df['Duration AS (s)'].mean(),3)), end='\n    * ')
print('Min: {}s'.format(round(df['Duration AS (s)'].min(),3)), end='\n    * ')
print('Max: {}s'.format(round(df['Duration AS (s)'].max(),3)), end='\n    * ')
print('Compared to baseline: {}%'.format(round(100*df['Duration AS (s)'].mean()/df['Duration BL (s)'].mean(),1)), end='\n    * ')
if intract_meth == True:
    print('Intractable cases: {} ({}%)'.format(n_intract_meth,round(100*n_intract_meth/test_pts,1)), end='\n * ')
else:
    print('No intractable cases', end='\n * ')
print('Number of LPs solved: {}'.format(int(df['Nb LPs AS'].mean())), end='\n * ')
print('Number of constraints (equalities): {}'.format(int(df['Nb cstr AS'].mean())), end='\n * ')
...
dur_M = pd.DataFrame({'Duration (s)':df['Duration AS (s)'], 'Test case':'{} bus'.format(n_bus), 'Solving method':'AllSets'})
...
gold = '#C48820'
blue = '#166FA0'
pal = {'Baseline': blue, 'AllSets': gold}
hue_order= ['Baseline', 'AllSets']
...
plt.savefig('Boxplot_AllSets_{}_{}.jpg'.format(n_bus,mode), format='jpg', dpi=1200)