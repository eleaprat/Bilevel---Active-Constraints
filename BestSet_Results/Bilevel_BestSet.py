...
def BestSet(n_bus, Demand, data, mode):
    ...
    if os.path.exists(name_AS):
        ...
        sys.exit('The list of active sets in csv is not available. Please add the file {} to the current directory or create it with the script BestSet_DB_DiscoverMass.py'.format(name_AS))
    ...
    dt_output=[]
    name_DT = 'BestSet_DT_{}bus.pkl'.format(n_bus)
    if os.path.exists(name_DT):
        if mode == 'basic':
            ...
            if os.path.exists('Classes_{}'.format(name_DT)):
                ...
                sys.exit('Please add the file {} to the directory and run again. If it does not exist, run BestSet_DT_Generation.py first. '.format('Classes_{}.pkl'.format(name_DT)))
        else:
            sys.exit('{} is not an accepted mode. Please select a proper mode: "basic" or "enhanced" and run again.'.format(mode))
    else:
        sys.exit('The decision tree {} does not exist. Run BestSet_DT_Generation.py first to create it.'.format(name_DT))
    ...
    duration_1 = time.time() - start_time # (can be parallelized)
    ...
    for Act_cstr in All_Sets:
        ...
        # Model alias
        model = gb.Model('BestSet_Bilevel_{}'.format(k))
    ...
    #-------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # model.write('BestSet_Bilevel_{}_Model.lp'.format(k)) # Equations
        # model.write('BestSet_Bilevel_{}_Results.sol'.format(k)) # Results
