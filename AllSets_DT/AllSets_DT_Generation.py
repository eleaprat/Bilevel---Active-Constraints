...
min_sp_split=2
min_sp_leaf=2
leaf = 500 # Max Leaves in DTs
feat=None
depth=10

# Mode: 0 for Hyperparameter tunning and 1 for running DT
mode = 0 # For general DT: First set to 0 to determine the best value for the hyperparameters, then change the hyperparameters above and set to 1
###############################################################################################################################
###############################################################################################################################
...
if os.path.exists(name_DB):
    ...
    sys.exit('The database in csv is not available. Please add the file {} to the current directory or create it with the script AllSets_DB_DiscoverMass.py'.format(name_DB))

# Create y array for the DT: x -> y, y being the set of active sets associated with each data point
...
### Create DT
name='AllSets_DT_{}bus'.format(n_bus)
...
if mode == 0:
    ...
    if os.path.exists(name_SAS):
        ...
        sys.exit('The list of sets of active sets in csv is not available. Please add the file or create it with the script AllSets_DB_DiscoverMass.py')
    ...
    print('There are {} different sets of sets of active constraints in the DB:'.format(len(SAS)))
    for s in SAS.keys():
        print(' - One set with {} sets of active constraints'.format(len(s)))
    print('----------------------------------------')
